...
fn test_min_stake() {
	new_test_ext().execute_with(|| {
		...
		register_module(netuid, U256::from(0), 0).ok();
		...
		for i in 1..n {
			...
			register_module(netuid, key, factor * min_stake).ok();
...
fn test_delegate_register() {
	new_test_ext().execute_with(|| {
		...
		for module_key in module_keys {
			delegate_register_module(netuid, key, module_key, stake_amount).ok();
...
fn test_many_registrations() {
	new_test_ext().execute_with(|| {
		...
		for i in 0..n {
			register_module(netuid, U256::from(i), stake).ok();
...
fn test_registration_with_stake() {
	new_test_ext().execute_with(|| {
		...
		for (i, stake) in stake_vector.iter().enumerate() {
			...
			register_module(netuid, key, stake_value).ok();
