...
fn test_stale_weights() {
	new_test_ext().execute_with(|| {
		...
		let vote_uid : u16  = 0 ;
		let mut weights = SubspaceModule::get_weights(netuid, vote_uid);

		subnet_params.max_weight_age = subnet_params.tempo as u64 - 1 ;
		SubspaceModule::set_subnet_params( netuid, subnet_params.clone());
		let uids = uids[1..11].to_vec();
		...
		let weight_age = SubspaceModule::get_weight_age(netuid, vote_uid);
		let new_weights = SubspaceModule::get_weights(netuid, vote_uid);
		let dividends = SubspaceModule::get_dividends(netuid);
		assert!(old_weights.len() > 0);
		assert!(new_weights.len() == 0);
		println!("old_weights: {:?}", old_weights);
		println!("new_weights: {:?}", new_weights);
		println!("subnet_params: {:?}", subnet_params);
...
fn test_max_weights() {
	new_test_ext().execute_with(|| {
		...
		let mut max_allowed_weights: u16 = 20;
		let mut min_allowed_weights : u16 = 1;
		let netuid: u16 = 0;
		register_n_modules(0, 100, 1000);
		let keys = SubspaceModule::get_keys(netuid);
		let uids = SubspaceModule::get_uids(netuid);
		...
		let mut subnet_params = SubspaceModule::subnet_params(netuid);
		subnet_params.max_allowed_weights = max_allowed_weights;
