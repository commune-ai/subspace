...
fn test_serve_not_registered() {
	new_test_ext().execute_with(|| {
		...
		let result = Subspace::serve_axon(<<Test as Config>::Origin>::signed(hotkey), version, ip, port, ip_type, modality );
...
fn test_serve_success() {
	new_test_ext().execute_with(|| {
		...
		register_ok_neuron(hotkey, coldkey);
		assert_ok!(Subspace::serve_axon(<<Test as Config>::Origin>::signed(hotkey), version, ip, port, ip_type, modality ));
        let neuron = Subspace::get_neuron_for_hotkey(&hotkey);
...
fn test_serve_success_with_update() {
	new_test_ext().execute_with(|| {
		...
		let port_2 = 1337;
		let modality_2 = 0;
        assert_ok!(Subspace::serve_axon(<<Test as Config>::Origin>::signed(hotkey), version_2, ip_2, port_2, ip_type_2, modality_2 ));
        let neuron2 = Subspace::get_neuron_for_hotkey(&hotkey);
		...
		// Check if the neuron has a hotkey account
		assert_eq!(Subspace::has_hotkey_account(&neuron2.uid), true);

		// Check if the balance of this hotkey account == 0
		assert_eq!(Subspace::get_stake_of_neuron_hotkey_account_by_uid(neuron2.uid), 0);
