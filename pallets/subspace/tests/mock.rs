...
pub fn register_ok_neuron( key_account_id: u64, key_account_id: u64) -> NeuronMetadata<u64> {
	...
	let (nonce, work): (u64, Vec<u8>) = subspace::create_work_for_block_number( block_number, (key_account_id + key_account_id) * 1000000 );
...
#[allow(dead_code)]
pub fn serve_axon( key_account_id : u64, version: u32, ip: u128, port: u16, ip_type : u8, modality: u8 ) -> NeuronMetadata<u64> {
	let result = subspace::serve_axon(<<Test as frame_system::Config>::Origin>::signed(key_account_id), version, ip, port, ip_type, modality );
	assert_ok!(result);
	let neuron = subspace::get_neuron_for_key(&key_account_id);
	neuron
}
