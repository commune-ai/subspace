...
fn test_registration_repeat_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(subspace::register(<<Test as Config>::Origin>::signed(key_account_id_1), block_number, nonce, work.clone(), key_account_id_1, key_account_id));
		let result = subspace::register(<<Test as Config>::Origin>::signed(key_account_id_2), block_number, nonce, work.clone(), key_account_id_2, key_account_id);
