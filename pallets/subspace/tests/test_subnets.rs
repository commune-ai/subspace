...
fn test_add_subnets() {
	new_test_ext().execute_with(|| {
		...
		for i in 0..num_subnets{
			...
			for j in 0..n {
				if j != i {
					let n = SubspaceModule::subnet_n(i);
			...
			if expected_subnets > max_allowed_subnets {
				...
				assert_eq!(SubspaceModule::subnet_n(i), n);
			}
			assert_eq!(
		...
		for netuid in 0..num_subnets {
			...
			assert_eq!(SubspaceModule::subnet_n(netuid), 0);
...
fn test_set_max_allowed_uids_growing() {
	new_test_ext().execute_with(|| {
		...
		for i in 1..max_uids {
			...
			assert_eq!(SubspaceModule::subnet_n(netuid), i + 1);
		}
		let mut n: u16 = SubspaceModule::subnet_n(netuid);
		let mut old_n: u16 = n.clone();
		let mut uids: Vec<u16>;
		assert_eq!(SubspaceModule::subnet_n(netuid), max_uids);
		let mut new_n: u16 = SubspaceModule::subnet_n(netuid);
		for r in 1..rounds {
			...
			n = SubspaceModule::subnet_n(netuid);
			assert_eq!(n, new_n);
...
fn test_set_max_allowed_uids_shrinking() {
	new_test_ext().execute_with(|| {
		...
		let mut n = SubspaceModule::subnet_n(netuid);
		...
		println!("n: {}", SubspaceModule::subnet_n(netuid));
