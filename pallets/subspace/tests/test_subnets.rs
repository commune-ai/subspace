...
fn test_add_subnets() {
	new_test_ext().execute_with(|| {
		...
		let max_allowed_subnets: u16 = SubspaceModule::getglobal_max_allowed_subnets();
		...
		for i in 0..num_subnets{
			let _ = register_module(i, U256::from(i), stake_per_module);
			for j in 0..n {
				if j != i {
					let n = SubspaceModule::get_subnet_n(i);
					println!("registering module i:{} j:{} n:{}", i,j,n);
					let _ = register_module(i, U256::from(j), stake_per_module);
				}
			}
			expected_subnets += 1;
...
// 		let _ = register_module(netuid, key, stake);
...
// 		let _result = SubspaceModule::set_subnet_params(
// 			netuid,
// 			params.clone()
// 		);
...
fn test_emission_ratio() {
	new_test_ext().execute_with(|| {
		...
		for i in 0..netuids.len() {
			...
			let delta: f64 = if subnet_emission > expected_emission {
				subnet_emission as f64 - expected_emission as f64
			} else {
				expected_emission as f64 - subnet_emission as f64
			};
...
// 		let rounds = 10;
// 		let _ = register_module(netuid, U256::from(0), stake);
...
// 				let _ = register_module(netuid, U256::from(uid), stake);
// 			}
...
// 		let _ = SubspaceModule::get_subnet_n(netuid);
