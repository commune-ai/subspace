...
fn set_weights_ok_no_weights() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Subspace::set_weights(Origin::signed(hotkey_account_id), weights_keys, weight_values));
		assert_eq!(Subspace::get_weights_for_neuron(&neuron), vec![u32::max_value()]);
		assert_eq!(Subspace::get_stake_of_neuron_hotkey_account_by_uid(neuron.uid), expect_stake);
...
fn test_priority_increments() {
	new_test_ext().execute_with(|| {
		...
		Subspace::add_stake_to_neuron_hotkey_account( neuron.uid, 2 );
		assert_eq!(Subspace::get_neuron_for_uid( neuron.uid ).priority, 0);
		assert_ok!(Subspace::set_weights(Origin::signed(hotkey_account_id), vec![], vec![]));
		assert_eq!(Subspace::get_neuron_for_uid( neuron.uid ).priority, 0);
        step_block (1);
        ...
        step_block (1);
		assert_eq!(Subspace::get_neuron_for_uid( neuron.uid ).priority, 1);
		Subspace::add_stake_to_neuron_hotkey_account( neuron.uid, 32 );
        step_block (1);
