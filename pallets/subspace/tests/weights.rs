...
fn test_set_weights_dispatch_info_ok() {
	new_test_ext().execute_with(|| {
		...
		let call = Call::subspace(subspaceCall::set_weights{dests, weights});
...
fn set_weights_ok_no_weights() {
	new_test_ext().execute_with(|| {
		...
		// == Intial values ==
		let key_account_id:u64 = 55; // Arbitrary number
		...
		let neuron = register_ok_neuron( key_account_id, 66);

		// Let's give it some stake.
		subspace::add_stake_to_neuron_key_account(neuron.uid, initial_stake);
...
fn test_priority_increments() {
	new_test_ext().execute_with(|| {
		let key_account_id:u64 = 55; // Arbitrary number
		let neuron = register_ok_neuron( key_account_id, key_account_id );
		subspace::add_stake_to_neuron_key_account( neuron.uid, 2 );
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 0);
		assert_ok!(subspace::set_weights(Origin::signed(key_account_id), vec![], vec![]));
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 0);
        step_block (1);
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 1);
        step_block (1);
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 2);
		assert_ok!(subspace::set_weights(Origin::signed(key_account_id), vec![], vec![]));
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 0);
        step_block (1);
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 1);
		subspace::add_stake_to_neuron_key_account( neuron.uid, 32 );
        step_block (1);
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 6);
        step_block (1);
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 11);
		assert_ok!(subspace::set_weights(Origin::signed(key_account_id), vec![], vec![]));
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 0);
        step_block (1);
		assert_eq!(subspace::get_neuron_for_uid( neuron.uid ).priority, 5);
	});
}
...
fn test_weights_err_weights_vec_not_equal_size() {
	new_test_ext().execute_with(|| {
		...
		let result = subspace::set_weights(Origin::signed(666), weights_keys, weight_values);
...
fn test_weights_err_max_weight_limit() {
	new_test_ext().execute_with(|| {
		...
		subspace::set_max_weight_limit(u32::MAX/5); // Set max to u32::MAX/5

		// Non self weight fails.
		let weights_keys: Vec<u32> = vec![1, 2, 3, 4]; 
		let weight_values: Vec<u32> = vec![1, 1, 1, 1]; // normalizes to u32::MAX/4
		let result = subspace::set_weights(Origin::signed(0), weights_keys, weight_values);
		...
		assert_ok!(subspace::set_weights(Origin::signed(0), weights_keys, weight_values));
	});
}
...
fn test_no_signature() {
	new_test_ext().execute_with(|| {
		...
		let result = subspace::set_weights(Origin::none(), weights_keys, weight_values);
...
fn test_set_weights_err_not_active() {
	new_test_ext().execute_with(|| {
		...
		let result = subspace::set_weights(Origin::signed(1), weights_keys, weight_values);
...
fn test_set_weights_err_invalid_uid() {
	new_test_ext().execute_with(|| {
		...
		let result = subspace::set_weights(Origin::signed(55), weight_keys, weight_values);
...
fn test_set_weight_not_enough_values() {
	new_test_ext().execute_with(|| {
		...
		subspace::set_min_allowed_weights(2);
		...
		let weight_values : Vec<u32> = vec![88]; // random value.
		let result = subspace::set_weights(Origin::signed(1), weight_keys, weight_values);
		...
		let weight_values : Vec<u32> = vec![88]; // random value.
		assert_ok!( subspace::set_weights(Origin::signed(1), weight_keys, weight_values)) ;
		...
		subspace::set_min_allowed_weights(1);
		assert_ok!( subspace::set_weights(Origin::signed(1), weight_keys, weight_values)) ;
	});
}
