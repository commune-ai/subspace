...
fn single_register_helper<T: Config>() -> (
    Vec<u8>,
    Vec<u8>,
    Vec<u8>,
    T::AccountId,
...
fn multiple_register_helper<T: Config>() -> (
    u16,
    Vec<T::AccountId>,
    Vec<u64>
) {
    ...
    let module_keys: Vec<T::AccountId> = (0..BATCH).map(|i| account("module_key", i as u32, SEED)).collect();
...
mod benchmarks {
    ...
    fn add_profit_shares() -> Result<(), BenchmarkError> {
        ...
        let keys: Vec<T::AccountId> = (0..BATCH).map(|i| account("key", i as u32, SEED)).collect();
        let shares: Vec<u16> = (0..BATCH).map(|i| i as u16 + 1).collect();
