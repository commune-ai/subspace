	...
	pub fn do_add_profit_shares(
		...
		if total_normalized_shares < u16::MAX {
			if normalize_shares.len() > 0 {
				normalize_shares[0] = normalize_shares[0]
					.saturating_add(u16::MAX.saturating_sub(total_normalized_shares));
			}
		...
		// TODO: check tssat the normalized shares add up to the unit
