...
const LOG_TARGET: &'static str = "runtime::subspace::serving";
...
impl<T: Config> Pallet<T> {
    pub fn do_serve_module( origin: T::Origin,  name: Vec<u8>,  ip: u128, port: u16 ) -> dispatch::DispatchResult {
        ...
        // --- We check if the key is registered, if not we register it.

        let key_id = ensure_signed(origin)?;

        // if Self::is_registered(&key_id) == false {
    ...
    pub fn do_registration (  origin: T::Origin ) -> dispatch::DispatchResult {
        ...
        if module_count < max_allowed_uids {
            ...
            Self::add_balance(&module_to_prune.key, stake_to_be_added_on_key.unwrap() );
        ...
        let module = ModuleMetadataOf::<T> {
            ...
            dividends: 0,
            url: vec![],
            ownership: u8::MAX/2 as u8, // 50% ownership.
            bonds: vec![],
            loans: vec![],
            stake_borrowed: 0, // No loans at registration.
            profit_share: 0,
    ...
    pub fn is_registered( key: &T::AccountId ) -> bool {
        return Keys::<T>::contains_key( key )
    }
