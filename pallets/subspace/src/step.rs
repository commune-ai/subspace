...
impl<T: Config> Pallet<T> {
	pub fn block_step() {
		...
		for (netuid, subnet_state) in SubnetStateStorage::<T>::iter() {
			...
			// increase the pending emission within the storage
			SubnetStateStorage::<T>::mutate(netuid, |proposal| {
	...
	pub fn epoch(netuid: u16) {
		...
		let mut total_stake_u64: u64 = subnet_state.total_stake;
		...
		let trust_ratio: u16 = subnet_params.trust_ratio.min(100);
		if trust_ratio > 0 {
		...
		// =================================
		// == Calculating Founder Share and Token Share==
		// =================================
		// FOUNDER DIVIDENDS 
		let founder_key = subnet_params.founder_key;
		let is_founder_registered = Self::is_key_registered(netuid, &founder_key);
		let founder_uid = u16::MAX;
		let mut founder_emission: u64 = 0;
		
		
		// WE NEED TO SUBTRACT THE FOUNDER SHARE FROM THE EMISSION BEFORE WE CALCULATE THE DIVIDENDS
		if is_founder_registered {
			let founder_share : u16 = subnet_params.founder_share;
			if founder_share > 0 {
		...
		// convert the emission to u64 for the incentives
		let mut incentive_ratio: I64F64 =  I64F64::from_num(subnet_params.incetive_ratio.min(100)) / I64F64::from_num(100);
		...
		let mut incentive_emission: Vec<u64> =
			...
			// convert the emission to u64 for the dividends
		...
		let burn_rate = global_params.burn_rate.min(100);
	...
	pub fn get_ownership_ratios(
		...
		if total_stake_from == I64F64::from_num(0) {
			ownership_vector.push((module_key.clone(), I64F64::from_num(1)));
		} else {
			ownership_vector =
