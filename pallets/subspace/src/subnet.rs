...
impl<T: Config> Pallet<T> {
	...
	pub fn if_module_name_exists(netuid: u16, name: Vec<u8>) -> bool {
		for (uid, _name) in <Name<T> as IterableStorageDoubleMap<u16, u16, Vec<u8>>>::iter_prefix(netuid) {
			if _name == name {
				return true
			}
		}
	...
	pub fn check_subnet_storage(netuid: u16) -> bool {
		...
		if (n as usize) != uids.len()
			|| (n as usize) != keys.len()
			|| (n as usize) != names.len()
			|| (n as usize) != addresses.len()
			|| (n as usize) != emissions.len()
			|| (n as usize) != incentives.len()
			|| (n as usize) != dividends.len()
			|| (n as usize) != last_update.len()
		{
			return false
		}
