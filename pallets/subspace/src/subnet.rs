...
impl<T: Config> Pallet<T> {
	...
	pub fn do_update_subnet(
		...
		ensure!(Self::get_vote_mode_subnet(netuid) == b"authority", Error::<T>::NotAuthorityMode);
		ensure!(Self::if_subnet_netuid_exists(netuid), Error::<T>::SubnetNameAlreadyExists);
		ensure!(Self::is_subnet_founder(netuid, &key), Error::<T>::NotFounder);
		ensure!(Self::if_subnet_netuid_exists(netuid), Error::<T>::SubnetNameAlreadyExists);
		ensure!(Self::is_subnet_founder(netuid, &key), Error::<T>::NotFounder);
	...
	pub fn check_subnet_params(params: SubnetParams<T>) -> DispatchResult {
		...
		// ensure the vode_mode is in "authority", "stake"
			...
			params.vote_mode == b"authority" || params.vote_mode == b"stake",
	...
	pub fn add_pending_deregistration_uid(netuid: u16, uid: u16) {
		...
		// TODO: handle error
		let _ = subnet_state.pending_deregister_uids.try_push(uid);
	...
	pub fn set_max_allowed_uids(netuid: u16, mut max_allowed_uids: u16) {
		...
		if max_allowed_uids < n {
			...
			let mut remainder_n = n - max_allowed_uids;
	...
	pub fn get_unit_emission() -> u64 {
		GlobalStateStorage::<T>::get().unit_emission
	}
	...
	pub fn get_total_emission_per_block() -> u64 {
		...
		let halving_total_stake_checkpoints = [10_000_000, 20_000_000, 30_000_000, 40_000_000]
			.iter()
			.map(|x| x * unit_emission);
		for (i, having_stake) in halving_total_stake_checkpoints.enumerate() {
	...
	pub fn if_subnet_name_exists(name: Vec<u8>) -> bool {
		SubnetStateStorage::<T>::iter().any(|(_, state)| state.name == name)
	}

	pub fn subnet_name_exists(name: Vec<u8>) -> bool {
		return Self::if_subnet_name_exists(name.clone())
	}
	...
	pub fn remove_subnet(netuid: u16) -> u16 {
		...
		let _ = Name::<T>::clear_prefix(netuid, u32::max_value(), None);
		let _ = Address::<T>::clear_prefix(netuid, u32::max_value(), None);
		...
		let _ = DelegationFee::<T>::clear_prefix(netuid, u32::max_value(), None);
		let _ = RegistrationBlock::<T>::clear_prefix(netuid, u32::max_value(), None);
	...
	pub fn set_vote_mode_subnet(netuid: u16, vote_mode: Vec<u8>) {
		...
		subnet_state.vote_mode =
			BoundedVec::<u8, ConstU32<32>>::try_from(vote_mode).expect("too long vote mode");
	...
	pub fn netuids() -> Vec<u16> {
		SubnetStateStorage::<T>::iter_keys().collect()
	}
	...
	pub fn set_last_update_for_uid(netuid: u16, uid: u16, last_update: u64) {
		...
		if (uid as usize) < updated_last_update_vec.len() {
			...
			subnet_state.last_updates =
				BoundedVec::<u64, ConstU32<10_000>>::try_from(updated_last_update_vec)
					.expect("too long vec");
	...
	pub fn set_max_allowed_weights(netuid: u16, max_allowed_weights: u16) {
		...
		subnet_state.max_allowed_weights =
	...
	pub fn check_subnet_storage(netuid: u16) -> bool {
		...
		if n != Self::get_uids(netuid).len() ||
			n != Self::get_keys(netuid).len() ||
			n != Self::get_names(netuid).len() ||
			n != Self::get_addresses(netuid).len() ||
			n != Self::get_emissions(netuid).len() ||
			n != Self::get_incentives(netuid).len() ||
			n != Self::get_dividends(netuid).len() ||
			n != Self::get_last_update(netuid).len()
		{
			return false;
		}
