...
impl<T: Config> Pallet<T> {
	...
	pub fn num_subnet_proposals(netuid: u16) -> u64 {
		Self::get_subnet_proposals(netuid).len() as u64
	}

	pub fn is_proposal_participant(key: &T::AccountId, proposal_id: u64) -> bool {
		Proposals::<T>::get(proposal_id).participants.contains(key)
	}
	...
	pub fn check_proposal(proposal: Proposal<T>) -> DispatchResult {
		...
		if mode == b"global" {
			...
			let subnet_params: SubnetParams<T> = Self::subnet_params(proposal.netuid);
				...
				subnet_params.vote_mode == b"stake" || subnet_params.vote_mode == b"authority",
	...
	pub fn is_proposal_owner(
		...
		!proposal.participants.is_empty() && proposal.participants[0] == *key
	}
	...
	pub fn check_proposal_approval(proposal_id: u64) {
		...
		if proposal.votes > stake_threshold {
			...
			} else if proposal.mode == b"global" {
	...
	pub fn get_subnet_proposals(netuid: u16) -> Vec<Proposal<T>> {
		Proposals::<T>::iter()
			.filter(|(_, proposal)| proposal.mode == b"subnet" && proposal.netuid == netuid)
	...
	pub fn get_global_proposals() -> Vec<Proposal<T>> {
		Proposals::<T>::iter()
			.filter(|(_, proposal)| proposal.mode == b"global")
			.map(|(_, proposal)| proposal)
			.collect()
	}

	pub fn num_global_proposals() -> u64 {
		Self::get_global_proposals().len() as u64
	}
	...
	pub fn is_vote_available(key: &T::AccountId, proposal_id: u64) -> bool {
		let proposal: Proposal<T> = Proposals::<T>::get(proposal_id);
		proposal.participants.contains(key) && !proposal.accepted
	}
}
