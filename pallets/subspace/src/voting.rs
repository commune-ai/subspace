...
impl<T: Config> Pallet<T> {
	pub fn proposal_global_update(
		...
		ensure!(last_id < u64::MAX, Error::<T>::TooMuchUpdateProposals);

		GlobalUpdateProposalLastId::<T>::mutate(|last_id| {
			*last_id = last_id.add(1);
		});

		GlobalUpdateProposals::<T>::insert(
			last_id,
	...
	pub fn stake_global_update(origin: T::RuntimeOrigin, proposal_id: u64) -> DispatchResult {
		let key = ensure_signed(origin)?;

		let last_proposal_id = GlobalUpdateProposalLastId::<T>::get();

		ensure!(proposal_id < last_proposal_id, Error::<T>::InvalidProposalId);
	...
	pub fn do_global_update(origin: T::RuntimeOrigin, proposal_id: u64) -> DispatchResult {
		ensure_signed(origin)?;
		...
		ensure!(proposal.votes > total_stake / 2, Error::<T>::NotEnoughVotesToAccept);
	...
	pub fn proposal_network_update(
		...
		ensure!(Self::if_subnet_netuid_exists(netuid), Error::<T>::SubnetNameNotExists);
		...
		SubnetUpdateProposals::<T>::insert(
			last_id,
	...
	pub fn stake_subnet_update(origin: T::RuntimeOrigin, proposal_id: u64) -> DispatchResult {
		let key = ensure_signed(origin)?;
	...
	pub fn do_subnet_update(origin: T::RuntimeOrigin, proposal_id: u64) -> DispatchResult {
		ensure_signed(origin)?;
	...
	pub fn is_vote_available(key: &T::AccountId, proposal_id: u64, is_global: bool) -> bool {
		if is_global {
	...
	pub fn get_total_global_stake(key: &T::AccountId) -> u64 {
