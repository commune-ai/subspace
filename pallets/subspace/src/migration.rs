...
use frame_support::{
	...
	weights::Weight,
	Blake2_128Concat, BoundedVec, Identity, Twox64Concat,
};
use log::info;
...
pub mod v1 {
	...
	#[storage_alias]
	pub(super) type Uids<T: Config> = StorageDoubleMap<
		Pallet<T>,
		Identity,
		u16,
		Blake2_128Concat,
	...
	#[storage_alias]
	pub(super) type Keys<T: Config> = StorageDoubleMap<
		Pallet<T>,
		Identity,
		u16,
		Identity,
	...
	pub(super) type Name<T: Config> =
		StorageDoubleMap<Pallet<T>, Twox64Concat, u16, Twox64Concat, u16, Vec<u8>>;
	...
	#[storage_alias]
	pub(super) type RegistrationBlock<T: Config> =
		StorageDoubleMap<Pallet<T>, Identity, u16, Identity, u16, u64>;
	...
	#[storage_alias]
	pub(super) type LoanTo<T: Config> = StorageMap<
		Pallet<T>,
		Identity,
		<T as frame_system::Config>::AccountId,
		Vec<(<T as frame_system::Config>::AccountId, u64)>,
	...
	#[storage_alias]
	pub(super) type LoanFrom<T: Config> = StorageMap<
		Pallet<T>,
		Identity,
		<T as frame_system::Config>::AccountId,
		Vec<(<T as frame_system::Config>::AccountId, u64)>,
	...
	#[storage_alias]
	pub(super) type ProfitShares<T: Config> = StorageMap<
		Pallet<T>,
		Identity,
		<T as frame_system::Config>::AccountId,
		Vec<(<T as frame_system::Config>::AccountId, u16)>,
	>;
	...
	pub(super) type Weights<T: Config> =
		StorageDoubleMap<Pallet<T>, Identity, u16, Identity, u16, Vec<(u16, u16)>>;
}
...
pub fn migrate_to_v2<T: Config>() -> Weight {
	...
	if onchain_version < 2 {
		...
		let global_state = GlobalState { registrations_per_block, total_subnets };
