...
pub mod v1 {
	...
	#[storage_alias]
	pub(super) type PendingDeregisterUids<T: Config> =
		StorageMap<Pallet<T>, Identity, u16, Vec<u16>>;

	#[storage_alias]
	pub(super) type Founder<T: Config> =
		StorageMap<Pallet<T>, Identity, u16, <T as frame_system::Config>::AccountId>;
...
pub fn migrate_to_v2<T: Config>() -> Weight {
	...
	if onchain_version < 2 {
		...
		let vote_mode =
			BoundedVec::<u8, ConstU32<32>>::try_from(v1::VoteModeGlobal::<T>::get().unwrap())
				.expect("too long vote mode");
		...
		for netuid in v1::MaxAllowedUids::<T>::iter_keys() {
			...
			let pending_deregister_uids = BoundedVec::<u16, ConstU32<10_000>>::try_from(
				v1::PendingDeregisterUids::<T>::get(netuid).unwrap(),
			)
			.expect("subnets exceed 10000");
			...
			let vote_mode = BoundedVec::<u8, ConstU32<32>>::try_from(
				v1::VoteModeSubnet::<T>::get(netuid).unwrap(),
			)
			.expect("too long vote mode");
			...
			let name = BoundedVec::<u8, ConstU32<32>>::try_from(
				v1::SubnetNames::<T>::get(netuid).unwrap(),
			)
			.expect("too long vote mode");
			let total_stake = v1::TotalStake::<T>::get(netuid).unwrap();
			let incentives = BoundedVec::<u16, ConstU32<10_000>>::try_from(
				v1::Incentive::<T>::get(netuid).unwrap(),
			)
			.expect("module count exceed 10000");
			let trusts =
				BoundedVec::<u16, ConstU32<10_000>>::try_from(v1::Trust::<T>::get(netuid).unwrap())
					.expect("module count exceed 10000");
			let dividends = BoundedVec::<u16, ConstU32<10_000>>::try_from(
				v1::Dividends::<T>::get(netuid).unwrap(),
			)
			.expect("module count exceed 10000");
			let emissions = BoundedVec::<u64, ConstU32<10_000>>::try_from(
				v1::Emission::<T>::get(netuid).unwrap(),
			)
			.expect("module count exceed 10000");
			let last_updates = BoundedVec::<u64, ConstU32<10_000>>::try_from(
				v1::LastUpdate::<T>::get(netuid).unwrap(),
			)
			.expect("module count exceed 10000");
