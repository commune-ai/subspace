...
use pallet_subspace::{params::subnet::DefaultSubnetParams, MaxAllowedSubnets, *};
...
fn test_subnet_eviction_when_max_reached() {
    new_test_ext().execute_with(|| {
        ...
        for i in 0..3 {
            ...
            <Test as SubnetEmissionApi<u32>>::set_subnet_emission_storage(
                i as u16,
                1000 - (i * 100) as u64,
            );
        }
        ...
        for i in 0..3 {
            let netuid =
                SubspaceMod::get_netuid_for_name(format!("protected-subnet-{}", i).as_bytes())
        ...
        for i in 3..max_subnets {
            let netuid =
                SubspaceMod::get_netuid_for_name(format!("removable-subnet-{}", i).as_bytes())
                    .expect("Subnet should exist");
            assert_eq!(
                <Test as SubnetEmissionApi<u32>>::get_subnet_consensus_type(netuid),
                Some(SubnetConsensus::Yuma)
        ...
        // Try to register a new subnet (should trigger eviction of the lowest emission removable
        // subnet)
        let mut new_params = DefaultSubnetParams::get();
        let new_subnet_name = b"new-test-subnet";
        new_params.name = BoundedVec::try_from(new_subnet_name.to_vec()).expect("Name too long");
...
fn test_protected_subnet_not_evicted() {
    new_test_ext().execute_with(|| {
        ...
        let mut new_params = DefaultSubnetParams::get();
        new_params.name = BoundedVec::try_from(b"new-subnet".to_vec()).expect("Name too long");
        new_params.founder = account_key("founder");
