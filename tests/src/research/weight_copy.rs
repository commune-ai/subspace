...
use pallet_offworker::{ConsensusSimulationResult, IrrationalityDelta};
use pallet_subnet_emission::{
    subnet_consensus::{
        util::params::{compute_weights, ConsensusParams},
        yuma::YumaEpoch,
    },
    SubnetConsensusType, Weights,
};
...
use pallet_subspace::{
    ...
    RegistrationBlock, Tempo, UseWeightsEncrytyption, ValidatorPermits, N,
};
...
fn test_backtest_simulation_with_copier_margins() {
    new_test_ext().execute_with(|| {
        ...
        for margin in 0..=9 {
            ...
            // copier will sets perfectly in consensus weight
            ...
            let (is_irra, _) = pallet_offworker::is_copying_irrational(simulation_result.clone());
            assert!(
                is_irra || iteration_counter >= MAX_EPOCHS,
...
fn run_simulation(
    ...
    for (block_number, block_weights) in json["weights"].as_object().unwrap() {
        ...
        let (is_irra, delta) = pallet_offworker::is_copying_irrational(simulation_result.clone());
        if is_irra {
            println!(
            ...
            // Save the delta
            IrrationalityDelta::<Test>::set(netuid, Some(delta));
    ...
    for uid in uids {
        if let Some(weights) = block_weights[&uid.to_string()].as_array() {
            ...
            if !zipped_weights.is_empty() && zipped_weights.iter().any(|(_, value)| *value != 0) {
                ValidatorPermits::<Test>::set(netuid, permits.clone());
